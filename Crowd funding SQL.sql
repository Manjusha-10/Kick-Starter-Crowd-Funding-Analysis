...
SELECT l.name, COUNT(p.ProjectID) AS project_count
FROM projects p
INNER JOIN location l ON p.location_id = l.location_id
GROUP BY l.name;

-- #Total Number of Projects based on  Category
SELECT c.name, COUNT(p.ProjectID) AS project_count
FROM projects p
...
GROUP BY 
    goal
ORDER BY 
    FIELD(goal, '0 to 5000', '5001 to 10000', '10001 to 25000', 'Over 25000');
